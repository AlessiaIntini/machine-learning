...
We represented the actDCF before calibration by applying the different models to the evaluation dataset and on different applications. We can already infer before calibration that the best model is the one that was chosen.
\begin{figure}[H]
    \centering
...
\begin{table}[H]
    ...
    \textbf{GMM} & \textcolor{red}{0.185387/0.189356 }& 0.183800/0.202290 & 0.183800/0.210571\\
    \hline
...
\end{figure}
\begin{itemize}
    \item After calculating minDCF and actDCF we can say that the results we get reflect the chosen model, in fact even after this analysis GMM gives us the best performance which is similar to the fusion of the different models. So we can say that we have made the right choice.
    \item Again the calibration strategy seems to have worked, in fact we can see that for the GMM model chosen with \(\tilde{\pi}=0.1\) we have a good calibration, minDCF and actDCF are very close in fact.
\end{itemize}
\section{Conclusions}

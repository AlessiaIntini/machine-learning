...
if __name__ == '__main__':
    ...
    if SAMEFIGPLOTS:
        ...
        axes = numpy.array([[plt.figure().gca(), plt.figure().gca(), plt.figure().gca()],
    ...
    print('\t\tminDCF(p=0.2), no cal.: %.3f' % bayesRisk.compute_minDCF_binary_fast(scores_sys_1, labels, 0.2, 1.0,
                                                                                    1.0))  # Calibration may change minDCF due to being fold-dependent (thus it's not globally affine anymore)
    print(
        '\t\tactDCF(p=0.2), no cal.: %.3f' % bayesRisk.compute_actDCF_binary_fast(scores_sys_1, labels, 0.2, 1.0, 1.0))
    ...
    print(
        '\t\tminDCF(p=0.2), cal.   : %.3f' % bayesRisk.compute_minDCF_binary_fast(calibrated_scores_sys_1, labels_sys_1,
                                                                                  0.2, 1.0,
                                                                                  1.0))  # Calibration may change minDCF due to being fold-dependent (thus it's not globally affine anymore)
    print(
        '\t\tactDCF(p=0.2), cal.   : %.3f' % bayesRisk.compute_actDCF_binary_fast(calibrated_scores_sys_1, labels_sys_1,
                                                                                  0.2, 1.0, 1.0))
    ...
    print('\t\tminDCF(p=0.2)         : %.3f' % bayesRisk.compute_minDCF_binary_fast(eval_scores_sys_1, eval_labels, 0.2,
                                                                                    1.0, 1.0))
    print('\t\tactDCF(p=0.2), no cal.: %.3f' % bayesRisk.compute_actDCF_binary_fast(eval_scores_sys_1, eval_labels, 0.2,
                                                                                    1.0, 1.0))
    print('\t\tactDCF(p=0.2), cal.   : %.3f' % bayesRisk.compute_actDCF_binary_fast(calibrated_eval_scores_sys_1,
                                                                                    eval_labels, 0.2, 1.0, 1.0))
    ...
    print('System 2')
    print('\tValidation set')
    print('\t\tminDCF(p=0.2), no cal.: %.3f' % bayesRisk.compute_minDCF_binary_fast(scores_sys_2, labels, 0.2, 1.0,
                                                                                    1.0))  # Calibration may change minDCF due to being fold-dependent (thus it's not globally affine anymore)
    print(
        '\t\tactDCF(p=0.2), no cal.: %.3f' % bayesRisk.compute_actDCF_binary_fast(scores_sys_2, labels, 0.2, 1.0, 1.0))
    ...
    print(
        '\t\tminDCF(p=0.2), cal.   : %.3f' % bayesRisk.compute_minDCF_binary_fast(calibrated_scores_sys_2, labels_sys_2,
                                                                                  0.2, 1.0,
                                                                                  1.0))  # Calibration may change minDCF due to being fold-dependent (thus it's not globally affine anymore)
    print(
        '\t\tactDCF(p=0.2), cal.   : %.3f' % bayesRisk.compute_actDCF_binary_fast(calibrated_scores_sys_2, labels_sys_2,
                                                                                  0.2, 1.0, 1.0))
    ...
    print('\tEvaluation set')
    print('\t\tminDCF(p=0.2)         : %.3f' % bayesRisk.compute_minDCF_binary_fast(eval_scores_sys_2, eval_labels, 0.2,
                                                                                    1.0, 1.0))
    print('\t\tactDCF(p=0.2), no cal.: %.3f' % bayesRisk.compute_actDCF_binary_fast(eval_scores_sys_2, eval_labels, 0.2,
                                                                                    1.0, 1.0))
    print('\t\tactDCF(p=0.2), cal.   : %.3f' % bayesRisk.compute_actDCF_binary_fast(calibrated_eval_scores_sys_2,
                                                                                    eval_labels, 0.2, 1.0, 1.0))
    ...
    print('Fusion')
    print('\tValidation set')
    print('\t\tminDCF(p=0.2)         : %.3f' % bayesRisk.compute_minDCF_binary_fast(fusedScores, fusedLabels, 0.2, 1.0,
                                                                                    1.0))  # Calibration may change minDCF due to being fold-dependent (thus it's not globally affine anymore)
    print('\t\tactDCF(p=0.2)         : %.3f' % bayesRisk.compute_actDCF_binary_fast(fusedScores, fusedLabels, 0.2, 1.0,
                                                                                    1.0))
    ...
    print('\tEvaluation set')
    print('\t\tminDCF(p=0.2)         : %.3f' % bayesRisk.compute_minDCF_binary_fast(fused_eval_scores, eval_labels, 0.2,
                                                                                    1.0, 1.0))
    print('\t\tactDCF(p=0.2)         : %.3f' % bayesRisk.compute_actDCF_binary_fast(fused_eval_scores, eval_labels, 0.2,
                                                                                    1.0, 1.0))

...
if __name__ == '__main__':
    ...
    print('\t\tactDCF(p=0.2), cal.   : %.3f' % (
        bayesRisk.compute_actDCF_binary_fast(calibrated_SVAL1, LVAL, 0.2, 1.0, 1.0)))
    ...
    # Evaluate the calibrated scores
    print('\t\tactDCF(p=0.2), cal.   : %.3f' % (
        bayesRisk.compute_actDCF_binary_fast(calibrated_SVAL2, LVAL, 0.2, 1.0, 1.0)))
    ...
    # The final model for single-fold systems corresponds to the model we trained on the calibration training set - we can directly apply it to application (evaluation) data
    fused_eval_scores = (
            w.T @ numpy.vstack([eval_scores_sys_1, eval_scores_sys_2]) + b - numpy.log(pT / (1 - pT))).ravel()
